...
# Having some real issues with the observes trying to check the values before they are there. It is seeing the values as NULL.
...
shinyApp(
  ...
  server = function(input, output, clientData, session){
    ...
    output$OP = renderUI({
      ...
      ### Power
      else{if(input$solvefor_OP == "Power"){
        list(
          # Inputs
          box(
            numericInput(inputId = "p_N_OP", label = "Proportion", min = 0, max = 1, value = 0.5, step = 0.01),
        ...
        ### Precision
        else{if(input$solvefor_OP == "Precision"){
          list(
            # Inputs
            box(
              numericInput(inputId = "p_N_OP", label = "Proportion", min = 0, max = 1, value = 0.5, step = 0.01),
              sliderInput(inputId = "p_S_OP", label = "", min = 0, max = 1, value = 0.5, step = 0.01),
              numericInput(inputId = "N_N_OP", label = "Sample Size", min = 0, max = 1000, value = 150),
              sliderInput(inputId = "N_S_OP", label = "", min = 0, max = 1000, value = 150),
              numericInput(inputId = "alpha_N_OP", label = "Significance Level", min = 0, max = 1, value = 0.05, step = 0.01),
              sliderInput(inputId = "alpha_S_OP", label = "", min = 0, max = 1, value = 0.05, step = 0.01)),
            # Outputs
            box(
              numericInput(inputId = "moe_N_OP", label = "+/- Precision", min = 0, max = 1, step = 0.01, value = 0.01),
              sliderInput(inputId = "moe_S_OP", label = "", min = 0, max = 1, step = 0.1, value = 0.01),
