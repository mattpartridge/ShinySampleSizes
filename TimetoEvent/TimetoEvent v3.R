    ...
    output$TTE = renderUI({
      list(
        ...
        box(width = ifelse(input$solvefor_TTE == "Sample Size", 4, 6),
            ...
            conditionalPanel("input.solvefor_TTE == 'Power'",
            ...
            conditionalPanel("input.solvefor_TTE == 'Sample Size'",
                             numericInput(inputId = "ctrlcensorrate_N_TTE",
                                          label = "Control Group's Censor Rate",
                                          value = 0,
                                          min = 0,
                                          max = 1,
                                          step = 0.01),
                             ...
                             numericInput(inputId = "expcensorrate_N_TTE",
                                          label = "Experimental Group's Censor Rate",
                                          value = 0,
                                          min = 0,
                                          max = 1,
                                          step = 0.01),
    ...
    observe({
      ...
      if(input$solvefor_TTE == "Sample Size"){
        ...
        # Sample Size Calculations
                ctrllambda = input$ctrllambda_N_TTE
                ctrlcensorrate = input$ctrlcensorrate_N_TTE
                lambda1 = ctrllambda*(ctrllambda/(ctrllambda + ctrlcensorrate))
                explambda = input$explambda_N_TTE
                expcensorrate = input$expcensorrate_N_TTE
                lambda2 = explambda*(explambda/(explambda + expcensorrate))
                Tstudy = input$studyduration_N_TTE
                ...
                gamma = ifelse(is.null(input$gamma_N_TTE), NA, input$gamma_N_TTE) # This could be a problem
                # N = ceiling(nSurvival(lambda1, lambda2, Tstudy, Tenrollment, eta, ratio, alpha, beta, sided, approx, type, entry, gamma)$n)
                N = ctrllambda
                n = N/2
                # Output
                ...
                updateSliderInput(session, inputId = "totalN_N_TTE", min = N - .2*N, value = N, max = N + .2*N)
      }
