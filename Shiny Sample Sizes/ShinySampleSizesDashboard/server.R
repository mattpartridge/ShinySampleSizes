...
server = function(input, output, clientData, session){
  ...
  output$Intro = renderUI({
    list(
      box(width = 12,
          h1("Shiny Dashboard for Sample Size and Power Calculations"),
          h3("Introduction"),
          h5("This is a Shiny Dashboard that can be used for sample size and power calculations for various study scenarios. These calculations are vital to the creation and analysis of clinical trials and scientific studies. There are many calculators available at differing price points, however very few offer an interactive experience. This Shiny Dashboard attempted to create an interactive sample size and power calculator allowing the user to explore and better understand the relationships between variables through interactivity. The resulting Shiny Dashboard incorporates interactive sliders and text boxes, a dynamic user interface, and robust statistical concepts to create a tool that is both functional and intuitive."),
          h5("For an in-depth discussion of how to use this dashboard, the underlying statistical theory for the calculations, the R functions that are used, examples, and more, refer to this dashboard's", a("documentation", href = "https://mfpartridge.shinyapps.io/shinysamplesizesdocumentation/", target = "_blank"), "."),
          h3("How to Use the Application"),
          tags$div(
            tags$ul(
              tags$li("Select a study scenario from the sidebar on the left"),
              tags$li("Determine what is being calculated at the top of each page"),
              tags$li("The page will update based on the calculation selection"),
              tags$li("Enter values for each of the inputs"),
              tags$li("The output will be displayed in the green box"))),
          h3("Contact Information"),
          h5("Matt Partridge"),
          h5("Masters Candidate in Biostatistics"),
          h5("University of Minnesota"),
          h5(a("LinkedIn", href = "https://www.linkedin.com/in/mattfpartridge/", target = "_blank")),
          h5(a("Please email Matt",  href = "mailto:mfpartridge92@gmail.com?subject=Shiny Dashboard for Sample Size and Power"), " for comments, questions, or suggestions regarding this dashboard.")))
  })
  ############################## Introduction ##############################
  ...
  output$TM = renderUI({
    ...
    list(
      ...
      box(width = 12,
          HTML(paste("The null (H", tags$sub(0), ") and alternative (H", tags$sub(1), ") hypotheses are:", tags$br(),
                     tags$strong("H", tags$sub(0)), ":  mean one - mean two = 0", tags$br(),
                     tags$strong("H", tags$sub(1)), ":  mean one - mean two &ne; 0"))),
      # Inputs
      box(
  ...
  output$TP = renderUI({
    list(
      ...
      box(width = 12,
          HTML(paste("The null (H", tags$sub(0), ") and alternative (H", tags$sub(1), ") hypotheses are:", tags$br(),
                     tags$strong("H", tags$sub(0)), ":  proportion one - proportion two = 0", tags$br(),
                     tags$strong("H", tags$sub(1)), ":  proportion one - proportion two &ne; 0"))),
      # Inputs
      box(
